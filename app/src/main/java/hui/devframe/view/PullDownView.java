...
public class PullDownView extends FrameLayout implements OnGestureListener, AnimationListener {
    ...
    LogUtils log = LogUtils.getLog("PullDownView");
    ...
    protected void addUpdateBar() {
        ...
        if (mIsLiveChat) {
            mRefreshView = (FrameLayout) LayoutInflater.from(getContext()).inflate(R.layout.common_listview_vw_update_bar_1, null);
        } else {
            mRefreshView = (FrameLayout) LayoutInflater.from(getContext()).inflate(R.layout.common_listview_vw_update_bar, null);
        }
